...
import com.truboard.framework.ExtentManager;
import com.truboard.framework.LogMe;
import com.truboard.setup.InitUtilObjects;
...
import com.truboard.utils.ExcelObject;
...
public class BaseTest {
	...
	public void suiteSetup() {
		try {
			...
			//TestDataConfig.getInstance().suiteSetup();
	...
	public void testResult(Method method, ITestResult result) throws IOException {
		...
		case ITestResult.SUCCESS:
			BaseTest.LOGGER.get().logTestStep(extentTest.get(), "PASS", "<b><------ Test case execution completed and successful for test:"+ BaseTest.LOGGER.get().test_name + " -----><b>", false);
			break;
		case ITestResult.FAILURE:
			BaseTest.LOGGER.get().logTestStep(extentTest.get(), "FAIL", "<b><------ Test case failed for test:"+ BaseTest.LOGGER.get().test_name + " -----><b>", true);
			break;
		case ITestResult.SKIP:
			BaseTest.LOGGER.get().logWithScreenshot("skip", "Test Case " + method.getName() + "  skiped", BaseTest.driver.get());
			break;
		default:
			break;
		}
